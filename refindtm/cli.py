...
def _resolve_refind_dir(args: argparse.Namespace) -> Path:
    ...
    if refind_dir is None:
        print(
            ...
            "  refindtm --refind-dir /boot/efi/EFI/refind list\n"
            "Belum pernah install rEFInd sama sekali? Coba 'refindtm setup' dulu.\n"
            "Jalankan 'refindtm doctor' untuk diagnostik lebih lanjut.",
            file=sys.stderr,
        )
        sys.exit(1)
    return refind_dir
...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    if not installed:
        print("Belum ada tema terpasang. Coba 'refindtm catalog' untuk melihat pilihan.")
        return
    ...
    if len(active_list) > 1:
        print(
            ...
            "jalankan 'refindtm activate <nama>' untuk merapikannya."
        )
...
def cmd_catalog(args: argparse.Namespace) -> None:
    ...
    print("Pasang salah satu dengan: refindtm install <key> --activate")
...
def cmd_install(args: argparse.Namespace) -> None:
    ...
    if args.activate:
        ...
        print(f"Jalankan 'refindtm activate {installed_name}' untuk mengaktifkannya.")
        ...
        if conf_path.is_file():
            ...
            if len(active_list) > 1:
                print(
                    ...
                    f"({', '.join(active_list)}). Jalankan 'refindtm activate <nama>' untuk merapikannya."
                )
...
def cmd_setup(args: argparse.Namespace) -> None:
    ...
    sendiri -- lihat refindtm/system.py. Tidak ada apa pun yang dijalankan tanpa
    ...
    if not system_mod.is_refind_install_available():
        ...
        if not args.yes:
            print(
                ...
                "Jalankan ulang dengan 'sudo refindtm setup --yes' untuk benar-benar memasangnya,\n"
                "atau jalankan perintah di atas secara manual."
            )
            return
        if not system_mod.is_root():
            print("Perintah ini butuh akses root. Jalankan ulang dengan: sudo refindtm setup --yes", file=sys.stderr)
            sys.exit(1)
        try:
    ...
    if not args.yes:
        print(
            "Ini baru pratinjau -- belum ada perubahan apa pun yang dibuat.\n"
            "Jalankan ulang dengan 'sudo refindtm setup --yes' untuk melanjutkan."
        )
        return
    if not system_mod.is_root():
        print("Perintah ini butuh akses root. Jalankan ulang dengan: sudo refindtm setup --yes", file=sys.stderr)
    ...
    refind_dir = detect_refind_dir(_refind_dir_arg(args))
    if refind_dir is not None:
        print(f"\nrEFInd berhasil terpasang di {refind_dir}. Coba 'refindtm doctor' untuk verifikasi.")
    else:
        print(
            "\nrefind-install selesai, tapi refindtm belum bisa mendeteksi lokasi rEFInd secara otomatis.\n"
            "Cek manual lokasi partisi EFI kamu, lalu jalankan: refindtm --refind-dir <path> doctor"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    #   refindtm --refind-dir /x list
    #   refindtm list --refind-dir /x
